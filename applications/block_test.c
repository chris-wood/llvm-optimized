#include <stdio.h>
#include <math.h>
#include <string.h>

#define COMPLEXITY 100000
#define LOOPS 1000
#define MOD 1000

long factorial(long n)
{
	if (n == 0 || n == 1)
		return 1;
	else
		return n * factorial(n - 1);
}

// Find all prime numbers up to CAP
int main(int argc, char** argv)
{
	unsigned int i, j;
	unsigned int cap = atoi(argv[1]);

	printf("Running block_test with %d iterations, %d mod split, %d loops, and %d factorial complexity\n", cap, MOD, LOOPS, COMPLEXITY);
	for (i = 0; i < cap; i++)
	{
		if ((i % MOD) == 0)
		{
			printf("Expensive step: %d\n", i);

			// Push a bunch of stuff onto the stack to mess up locality of reference...
			unsigned char s1[256] = 
			{
			   
			};
			unsigned char s2[256] = 
			{
			   
			};
			unsigned char s3[256] = 
			{
			   
			};
			unsigned char s4[256] = 
			{
			   
			};
			unsigned char s5[256] = 
			{
			   
			};
			unsigned char s6[256] = 
			{
			   
			};
			unsigned char s7[256] = 
			{
			   
			};
			unsigned char s8[256] = 
			{
			   
			};
			unsigned char s9[256] = 
			{
			   
			};
			unsigned char s10[256] = 
			{
			   
			};
			for (j = 0; j < LOOPS; j++) factorial(COMPLEXITY);
		}
		else
		{
			printf("Simple step: %d\n", i);
		}
	}
	return 0;
}
